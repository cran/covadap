...
DABCD.sim <- function(data = NULL, covar = NULL, n = NULL, all.cat, nrep = 1000,
                      print.results = TRUE) {
  if(missing(all.cat)){
  ...
  if(all.cat){
    ...
    invisible(res)
  }else{ # if mixed covariates are present

    if (is.null(data)) {  #if no data are inserted in input
      ...
      if(length(type.cov) == 2){
        ...
        quant.par = quant.par = matrix(unlist(type.quant), ncol = 2, byrow = T)
        ...
        for (r in 1:nrep) {
          ...
          strata = sample(1:n.strata, n, T)
          if(length(n_levels) ==1 ){
            Fn = cbind(num.strata[strata,], matrix(rnorm(n_cov_quant*n, mean = quant.par[,1], sd = quant.par[,2]), nrow = n, byrow = T ))
          } else{
        ...
        type.quant = covar[[which(type.cov == 2)]]
        n_cov_quant = length(type.quant)
        quant.par = matrix(unlist(type.quant), ncol = 2, byrow = T)
        n_cov_tot = length(var_names)
        res = res.data = list()
        Imb.measures = matrix(NA, nrow = nrep, ncol = 3)
        dif.cont = matrix(NA, nrow = nrep, ncol = n_cov_quant)
      ...
      invisible(res)
    } else{  ###se vengono forniti i dati
      res = res.data = list()
